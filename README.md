...
이 중에서도 **빠른 시작시간 작업**을 우선한 알고리즘으로 작업스케줄링을 설계했다. 
...
입력받은 n을 맞는 기계에 배정해주는 O(m)의 시간, while 루프가 입력만큼 수행되는 시간O(n)을 곱해줘 O(mn)의 시간이 걸린다.</br></br>
결국, 작업스케줄링에 **시간 복잡도**는 __O(nlogn)+O(mn)__이 걸리게 된다.</br>
...
public class JobSchedule {
    public static void main(String[] args)
    {
        ...
        int n=4,machine=0; // n개의 입력을 받으므로 machine의 개수는 최대 4개까지 가능하다.
        int [] mc=new int[4]; //실행될 기계
        int [][] job= { {2, 5 ,0}, {1, 4, 0}, {6, 9, 0}, {4, 6, 0} };
